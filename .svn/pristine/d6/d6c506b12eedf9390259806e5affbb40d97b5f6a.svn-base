...
<div class="ui-jqdialog-content ui-widget-content" id="ykpzInfo" style="height: 98%;">
		<form class="FormGrid" onsubmit="return false;" style="width: auto; height: auto;">
			<fieldset>
			<legend>移库凭证信息</legend>
			<table id="ykpzInfo" class="EditTable ui-common-table" >
				<tbody>
			...
			<fieldset>
			<legend>移库明细</legend>
			<table id="ykmxjqGrid"></table>
			<div id="ykmxjqGridPager"></div>
			</fieldset>
			...
			<table style="height: auto" class="EditTable ui-common-table">
				<tbody>
					...
					<tr>
						...
						<td class="EditButton">
							<a id="addYkData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" >保存<span class="ui-icon ui-icon-disk"></span></a>
							<a id="celYkData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="closeYkdWin();">取消<span class="ui-icon ui-icon-close"></span></a>
						</td>
					</tr>
				</tbody>
		  </table>
...
$(function(){
  ...
  $('#ykmxjqGrid').jqGrid({
		...
		autowidth: false,
		width:$('#ykpzInfo').width()*0.999,
		height:$('#ykpzInfo').height()*0.5,
		shrinkToFit: true, 
		altRows :true,
		altclass: 'someClass',
		onSelectRow: editRow,
		rownumbers: true,
		rowNum: 20,
		pager: "#ykmxjqGridPager"
	});
  	$('#ykmxjqGrid').navGrid('#ykmxjqGridPager',
    ...
    function editRow(id) {
  	...
  	//弹出新增移库明细窗口
  	$('#add_ykmxjqGrid').on('click',function(){
  		$( "#addYkMxWin" ).dialog('open');
  		$( "#addYkMxWin" ).parent().prev('.ui-widget-overlay').css({'z-index':'950'});
		$( "#addYkMxWin" ).parent().css({'z-index':'951'});
    	$('#addYkMxWin').load(ctx+'/wmspage/ykpzmanage/ykpzgl/addykmx.jsp');
  	});
  	  	
  	$('#addYkData').hover(function(){
		$(this).addClass("ui-state-hover");},
		function(){ 
			$(this).removeClass("ui-state-hover");}
		);
  	$('#celYkData').hover(function(){
		$(this).addClass("ui-state-hover");},
		function(){ 
			$(this).removeClass("ui-state-hover");}
		);
});
function closeYkdWin(){
	$('#addYkdWin').dialog('close');
}
</script>
